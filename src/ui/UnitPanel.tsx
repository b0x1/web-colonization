...
export const UnitPanel: React.FC = () => {
  ...
  const settlementAtTile = selectedTile
    ? players
        ...
        .find((s) => s.x === selectedTile.x && s.y === selectedTile.y)
    : null;
  ...
  if (
    ...
    settlementAtTile &&
    player &&
    settlementAtTile.ownerId === player.id
  ) {
  ...
  if (
    !unit &&
    selectedTile &&
    (unitsAtTile.length > 1 ||
      (settlementAtTile &&
        (unitsAtTile.length > 0 ||
          (player && settlementAtTile.ownerId === player.id))))
  ) {
    return (
      <UnitSelector
  ...
  const isAdjacentToSettlement = allSettlements.some(
    (s) => Math.abs(s.x - unit.x) <= 1 && Math.abs(s.y - unit.y) <= 1,
  );
  ...
  return (
    <div className="absolute bottom-5 left-5 w-64 bg-black/80 text-white p-5 rounded-xl pointer-events-auto shadow-2xl border border-white/10 backdrop-blur-sm font-sans">
      <h3 className="text-xl font-black uppercase tracking-tight mb-1 text-blue-400">
        Unit: {unit.type}
      </h3>
      {unit.specialty && (
        <div className="text-[10px] font-black uppercase tracking-widest text-yellow-500 mb-1 bg-yellow-900/40 px-1.5 py-0.5 rounded border border-yellow-800/30 w-fit">
          Expert {unit.specialty}
        </div>
      )}
      {isReadOnly && (
        <div className="text-[10px] font-black uppercase tracking-widest text-red-500 mb-2">
          [READ ONLY - {unitOwner?.name}]
        </div>
      )}
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-[10px] font-mono font-bold text-slate-400">
          {unit.movesRemaining}/{unit.maxMoves} MOVES
        </span>
      </div>
      ...
      <div className="text-xs bg-slate-900/50 p-3 rounded-lg border border-white/5 mb-4">
        ...
        {unit.cargo.size === 0 ? (
          <span className="text-slate-600 italic text-center block py-1">
            Empty Cargo Hold
          </span>
        ) : (
        ...
        <div className="space-y-2">
          {(unit.type === UnitType.COLONIST ||
            ...
            <div className="relative group/tooltip">
              ...
              >
                <span className="text-yellow-400 font-black">B</span>UILD
                SETTLEMENT
              </button>
              {isAdjacentToSettlement && (
