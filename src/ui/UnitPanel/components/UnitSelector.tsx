...
export const UnitSelector: React.FC<Props> = ({
  ...
  return (
    <div className="absolute bottom-5 left-5 w-64 bg-black/80 text-white p-5 rounded-xl pointer-events-auto shadow-2xl border border-white/10 backdrop-blur-sm font-sans">
      <h3 className="text-xl font-black uppercase tracking-tight mb-4 text-blue-400">
        Tile Selection
      </h3>
      <div className="space-y-2 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
        {settlementAtTile && (
          ...
          >
            <div
              className={`w-10 h-10 ${isOwnedSettlement ? 'bg-blue-900/60 border-blue-400/30 group-hover:border-blue-300/50' : 'bg-slate-900/60 border-slate-400/30 group-hover:border-slate-300/50'} rounded flex items-center justify-center border`}
            >
              <span
                className={`text-xs font-black ${isOwnedSettlement ? 'text-blue-300' : 'text-slate-300'}`}
              >
                S
              </span>
            </div>
            <div>
              <div
                className={`text-xs font-black uppercase tracking-widest ${isOwnedSettlement ? 'text-blue-200' : 'text-slate-200'}`}
              >
                {settlementAtTile.name}
              </div>
              <div
                className={`text-[10px] ${isOwnedSettlement ? 'text-blue-400' : 'text-slate-400'} font-bold uppercase tracking-tight`}
              >
                Settlement
              </div>
            </div>
