      ...
      <table className="w-full text-left border-collapse">
        <thead>
          <tr className="border-b border-slate-600 bg-slate-800/50">
            <th className="p-3 font-bold sticky left-0 bg-slate-800 z-10">
              Flag
            </th>
            <th className="p-3 font-bold sticky left-[64px] bg-slate-800 z-10">
              Settlement
            </th>
            {goods.map((good) => (
              <th
          ...
          {displayedPlayers.map((player) =>
            player.settlements.map((settlement) => (
              <tr
                ...
                onClick={() =>
                  onSettlementClick(settlement.id, settlement.x, settlement.y)
                }
              >
