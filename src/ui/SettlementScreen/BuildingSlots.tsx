...
const BUILDINGS_LIST = [
  ...
  {
    type: BuildingType.CARPENTERS_SHOP,
    name: "Carpenter's Shop",
    bonus: 'Lumber -> Hammers',
  },
  {
    type: BuildingType.BLACKSMITHS_HOUSE,
  ...
  {
    type: BuildingType.WEAVERS_SHOP,
    name: 'Weaver Shop',
    bonus: 'Cotton -> Cloth',
  },
  {
    type: BuildingType.TOBACCONISTS_SHOP,
    name: 'Tobacconist',
    bonus: 'Tobacco -> Cigars',
  },
...
export const BuildingSlots: React.FC<Props> = ({
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
      {BUILDINGS_LIST.filter((b) => ownedBuildings.includes(b.type)).map(
        (b) => {
          const associatedJob = Object.values(JOB_PRODUCTION_RULES).find(
            (rule) => rule.requiredBuildings.includes(b.type),
          )?.jobType;
          ...
          return (
            ...
            >
              <div>
                <div className="font-black text-xs uppercase tracking-widest text-blue-400 mb-1">
                  {b.name}
                </div>
