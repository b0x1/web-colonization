...
export const AvailableUnits: React.FC<Props> = ({ settlementId, units }) => {
  ...
  return (
    ...
    >
      <h3 className="text-lg font-black uppercase tracking-tight mb-4 text-slate-300">
        Available Units
      </h3>
      <div className="flex gap-4 min-w-max pr-4">
          ...
          return (
            ...
            >
              ...
              <div className="text-center w-full">
                <div className="text-[10px] font-black uppercase tracking-widest text-blue-400 truncate w-full">
                  {unit.type}
                </div>
                {unit.specialty && (
                  <div className="text-[8px] text-yellow-500 font-black uppercase tracking-tighter truncate w-full">
                    Expert {unit.specialty}
                  </div>
                )}
                <div className="text-[8px] text-slate-500 font-mono mt-0.5">
                  ID: {unit.id.slice(0, 4)}
                </div>
              </div>
            </div>
          );
        ...
        {availableUnits.length === 0 && (
          <div className="text-slate-600 italic text-sm self-center">
            No units available
          </div>
        )}
      </div>
