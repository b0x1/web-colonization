...
export const FieldPanel: React.FC = () => {
  ...
  const unitsAtTile = allUnits.filter(
    (u) => u.x === selectedTile.x && u.y === selectedTile.y,
  );
  const showAboveUnitPanel = selectedUnitId || unitsAtTile.length > 1;

  return (
    <div
      className={`absolute left-5 w-64 bg-black/80 text-white p-4 rounded-xl pointer-events-auto shadow-2xl border border-white/10 backdrop-blur-sm font-sans mb-2 ${showAboveUnitPanel ? 'bottom-[calc(20px+16rem)]' : 'bottom-5'}`}
    >
      ...
      <div className="space-y-1.5 text-xs">
        <div className="flex justify-between">
          ...
          <span className="font-bold text-blue-300 capitalize">
            {tile.hasResource
              ? tile.hasResource.toLowerCase().replace('_', ' ')
              : 'None'}
          </span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <span
            className={`font-bold ${defenseBonus >= 0 ? 'text-green-400' : 'text-red-400'}`}
          >
            {defenseBonus > 0 ? '+' : ''}
