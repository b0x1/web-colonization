...
export const InventoryPanel: React.FC<Props> = ({
  ...
  const calculateProduction = (good: GoodType) => {
    ...
    if (
      ...
      buildings.includes(BuildingType.LUMBER_MILL)
    )
      prod += 2;
    if (good === GoodType.ORE && buildings.includes(BuildingType.IRON_WORKS))
      prod += 2;
        ...
        <tbody>
          {Object.values(GoodType).map((good) => {
            ...
            return (
              ...
              >
                <td className="py-2 text-slate-300 font-medium capitalize">
                  {good.toLowerCase().replace('_', ' ')}
                </td>
                <td className="py-2 font-mono font-bold">{stock}</td>
                <td
                  className={`py-2 text-right font-mono font-bold ${net > 0 ? 'text-green-400' : net < 0 ? 'text-red-400' : 'text-slate-500'}`}
                >
                  {net > 0 ? '+' : ''}
                  {net}
                </td>
