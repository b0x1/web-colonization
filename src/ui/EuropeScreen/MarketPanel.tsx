...
export const MarketPanel: React.FC = () => {
  ...
  return (
    <div className="bg-slate-900/40 p-6 rounded-xl border border-slate-700 shadow-inner">
      <h3 className="text-2xl font-black uppercase tracking-tight mb-4 text-blue-300">
        Market Prices
      </h3>
        ...
        <tbody>
          {Object.values(GoodType).map((good) => {
            ...
            return (
              <tr
                key={good}
                className="border-b border-slate-800 hover:bg-slate-700/20 transition-colors"
              >
                    ...
                    <div className="flex items-center gap-1 ml-4 bg-slate-800 p-1 rounded border border-slate-600">
                      ...
                      <button
                        onClick={() => {
                          ...
                          if (amount > 0)
                            buyGood(selectedUnit.id, good, amount);
                        }}
