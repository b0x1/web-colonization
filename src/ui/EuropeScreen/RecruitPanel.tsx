...
export const RecruitPanel: React.FC = () => {
  const { players, currentPlayerId, selectedUnitId, recruitUnit } =
    useGameStore();
  ...
  return (
    <div className="bg-slate-900/40 p-6 rounded-xl border border-slate-700 shadow-inner">
      <h3 className="text-2xl font-black uppercase tracking-tight mb-6 text-blue-300">
        Recruit Units
      </h3>
      <div className="grid grid-cols-3 gap-6">
        {unitsToRecruit.map((u) => {
          ...
          const hasMuskets =
            u.type !== UnitType.SOLDIER ||
            (selectedUnit.cargo.get(GoodType.MUSKETS) || 0) >= 50;
          ...
          return (
            <div
              ...
              className={`p-5 rounded-lg border flex flex-col items-center gap-3 transition-all ${
                canRecruit
                  ? 'bg-slate-800 border-slate-600 hover:border-blue-500 shadow-lg'
                  : 'bg-slate-900/50 border-slate-800 opacity-75'
              }`}
            >
              <h4 className="text-lg font-black uppercase tracking-widest text-slate-200">
                {u.type}
              </h4>
