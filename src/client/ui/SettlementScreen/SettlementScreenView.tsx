...
export interface SettlementScreenViewProps {
  settlement: Settlement;
  player: Player;
  settlementOwner: Player;
  isReadOnly: boolean;
  hammersProduced: number;
  unitsAtSettlement: Unit[];
  netProduction: Map<GoodType, number>;
  onClose: () => void;
}

export const SettlementScreenView: React.FC<SettlementScreenViewProps> = ({
  settlement,
  player,
  settlementOwner,
  isReadOnly,
      ...
      <div className="flex-1 grid grid-cols-[2fr_1fr] grid-rows-[1fr_auto] gap-4 p-4 overflow-hidden">
        ...
        <div className="flex flex-col gap-4 overflow-hidden">
          ...
          <div className="h-48 bg-slate-900/30 rounded-xl border border-slate-800 p-4">
            <InventoryPanel settlement={settlement} netProduction={netProduction} />
          </div>
        </div>
