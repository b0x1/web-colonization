...
import { UnitType, Attitude } from '@shared/game/entities/types';
import { isSame, type Position } from '@shared/game/entities/Position';
import { TraversalUtils } from '@shared/game/utils/TraversalUtils';
import { CAMERA_CONSTANTS } from '@shared/game/constants';
...
export class InputHandler {
  ...
  setup(mapWidth: number, mapHeight: number, getReachableTiles: () => Position[]): void {
    ...
    this.pointerDownHandler = (pointer: Phaser.Input.Pointer) => {
      ...
      if (pointer.leftButtonDown()) {
        ...
        this.handleRightClick(pos, getReachableTiles());
      }
    };
  ...
  private handleRightClick(pos: Position, reachableTiles: Position[]) {
    const state = useGameStore.getState();
    ...
    if (selectedUnit.type === UnitType.SOLDIER && foreignUnitAtTile ) {
      ...
      if (isReachable) {
        eventBus.emit('moveUnitRequested', { id: state.selectedUnitId, to: pos });
      } else {
        eventBus.emit('unitSelected', null);
        this.terrainRenderer.updateSelectionHighlight(null);
      }
