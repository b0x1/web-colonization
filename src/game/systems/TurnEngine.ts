...
export class TurnEngine {
  ...
  static runProduction(players: Player[], map: Tile[][]): Player[] {
    const updatedPlayers = players.map((player) => {
      const newPlayerUnits = player.units.map((u) => ({
        ...u,
        cargo: new Map(u.cargo),
      }));
      ...
      const newSettlements = player.settlements.map((settlement) => {
        ...
        newSettlement.workforce.forEach((assignment, unitId) => {
          ...
          unit.turnsInJob += 1;
          if (
            unit.turnsInJob >= COLONY_CONSTANTS.EXPERT_PROMOTION_TURNS &&
            !unit.specialty
          ) {
            if (Object.values(JobType).includes(assignment as JobType)) {
              ...
              eventBus.emit(
                'notification',
                `${unit.type} has become an expert ${unit.specialty}!`,
              );
            }
          }
        });
        ...
        const { netProduction, hammersProduced } =
          ProductionSystem.calculateSettlementProduction(
            newSettlement,
            map,
            true,
          );
        ...
        netProduction.forEach((amount, good) => {
          newSettlement.inventory.set(
            good,
            Math.max(0, (newSettlement.inventory.get(good) || 0) + amount),
          );
        });
        ...
        if (newSettlement.productionQueue.length > 0) {
          ...
          const isBuilding = Object.values(BuildingType).includes(
            currentItem as BuildingType,
          );
          const isUnit = Object.values(UnitType).includes(
            currentItem as UnitType,
          );

          const cost = isBuilding
            ? BUILDING_COSTS[currentItem as string] || { hammers: 40, tools: 0 }
            : UNIT_BUILD_COSTS[currentItem as string] || {
                hammers: 40,
                tools: 0,
                muskets: 0,
              };
          ...
          const currentMuskets =
            newSettlement.inventory.get(GoodType.MUSKETS) || 0;
          ...
          if (currentTools >= toolsNeeded && currentMuskets >= musketsNeeded) {
            if (newSettlement.hammers >= cost.hammers) {
              ...
              newSettlement.inventory.set(
                GoodType.TOOLS,
                currentTools - toolsNeeded,
              );
              newSettlement.inventory.set(
                GoodType.MUSKETS,
                currentMuskets - musketsNeeded,
              );
              ...
              if (isBuilding) {
                ...
                eventBus.emit(
                  'notification',
                  `${newSettlement.name} completed ${currentItem}!`,
                );
              } else if (isUnit) {
        ...
        if (netFood >= COLONY_CONSTANTS.FOOD_GROWTH_THRESHOLD) {
          newSettlement.inventory.set(
            GoodType.FOOD,
            netFood - COLONY_CONSTANTS.FOOD_GROWTH_THRESHOLD,
          );
          newSettlement.population += 1;
          ...
          eventBus.emit(
            'notification',
            `A new colonist has been born in ${newSettlement.name}!`,
          );
        } else {
