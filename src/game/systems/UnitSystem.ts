...
export class UnitSystem {
  static findNextAvailableUnit(
    ...
    const availableUnits = player.units.filter(
    ...
    return availableUnits.reduce((prev, curr) => {
      ...
      const distPrev =
        Math.abs(prev.x - currentUnit.x) + Math.abs(prev.y - currentUnit.y);
      const distCurr =
        Math.abs(curr.x - currentUnit.x) + Math.abs(curr.y - currentUnit.y);
