...
describe('NativeInteractionSystem', () => {
  ...
  it('should throw error when non-colonist tries to learn', () => {
    ...
    expect(() =>
      NativeInteractionSystem.learn(mockSettlement, soldier),
    ).toThrow();
  });
  ...
  it('should throw error when learning from non-friendly settlement', () => {
    ...
    expect(() =>
      NativeInteractionSystem.learn(neutralSettlement, mockUnit),
    ).toThrow();
  });

  it('should shift attitude correctly', () => {
    expect(NativeInteractionSystem.shiftAttitude(Attitude.FRIENDLY)).toBe(
      Attitude.NEUTRAL,
    );
    expect(NativeInteractionSystem.shiftAttitude(Attitude.NEUTRAL)).toBe(
      Attitude.HOSTILE,
    );
    expect(NativeInteractionSystem.shiftAttitude(Attitude.HOSTILE)).toBe(
      Attitude.HOSTILE,
    );
  });
});
